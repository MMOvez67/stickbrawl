...
const tickMuzzles=()=>{muzzles=muzzles.filter(m=>m.life>0);muzzles.forEach(m=>m.life--);};
const drawMuzzles=()=>{
...
const tickShells=()=>{shells=shells.filter(s=>s.life>0);shells.forEach(s=>s.tick());};
...
const tickDnums=()=>{dNums=dNums.filter(d=>d.life>0);dNums.forEach(d=>{d.y+=d.vy;d.vy*=.93;d.life-=.024;});};
