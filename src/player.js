...
class Player{
  ...
  headBox(){return{x:this.x-10,y:this.y-72,w:20,h:22};}
  punchBox(){
  ...
  tick(wpns,thrownWpns,explosions){
    ...
    for(const b of this.bullets){
      ...
      for(const p of map.plats){if(pInBox(b.x,b.y,p)){explosions.push(new Explosion(b.x,b.y,100));addPts(b.x,b.y,'#ff6600',22,9,6,.12);addShake(8,12);sound('rocket');b.active=false;break;}}
    }
  ...
  _doAttack(){
    ...
    else if(this.weapon==='SNIPER'){this.bullets.push(new Bullet(bx,by,this.facing*28,0,this.id,'sniper'));addMuzzle(bx,by,this.facing);this.vx-=this.facing*3.5;this._useAmmo(30,30);sound('sniper');addShake(5,8);}
