...
class Explosion{
  ...
  tick(dt=1){this.r=lerp(this.r,this.maxR,.22*dt);this.life-=.048*dt;if(!this.peaked&&this.r>this.maxR*.45)this.peaked=true;if(this.life<=0)this.active=false;}
...
class Bullet{
  ...
  tick(dt=1){
    ...
    const gravMap={rocket:.055,sniper:.008,bouncer:.035,pellet:.10,minigun:.07,normal:.025};
    this.vy+=(gravMap[this.type]||.025)*dt*s;
    this.x+=this.vx*dt*s;this.y+=this.vy*dt*s;this.life-=dt;
    ...
    for(const p of map.plats){
      ...
      if(this.type==='bouncer'&&this.bounces<5){const fl=this.vx>0&&this.x<p.x+20,fr=this.vx<0&&this.x>p.x+p.w-20;if(fl||fr)this.vx*=-1;else this.vy*=-1;this.bounces++;this.x+=this.vx*2*dt;this.y+=this.vy*2*dt;addImpactSparks(this.x,this.y,this.vx,this.vy);return;}
...
class BlinkStrike{
  ...
  tick(dt=1){this.t+=dt;if(this.t>=this.dur)this.active=false;}
