...
class Grenade{
  ...
  tick(explosions){this.vy+=GRAV*.7;this.vx*=.992;this.x+=this.vx;this.y+=this.vy;this.angle+=this.vx*.08;this.fuse--;if(this.fuse<=0){this._explode(explosions);return;}for(const p of map.plats){if(!pInBox(this.x,this.y,p))continue;if(this.vy>0){this.y=p.y-1;this.vy*=-.55;this.vx*=.7;}else{this.y=p.y+p.h+1;this.vy*=-.4;}this.bounces++;if(this.bounces>4)this._explode(explosions);break;}if(this.y>H+60)this.active=false;}
...
class StickyBomb{
  ...
  tick(players,exp){
    this.fuse--;
    if(!this.stuck){this.vy+=GRAV*.6;this.x+=this.vx;this.y+=this.vy;for(const p of map.plats){if(pInBox(this.x,this.y,p)){this.stuck=true;this.vx=0;this.vy=0;break;}}for(const p of players){if(!p.active||!p.alive||p.id===this.owner)continue;if(pInBox(this.x,this.y,p.box())){this.stuck=true;this.stuckTo=p;this.vx=0;this.vy=0;break;}}}
...
class ThrownWpn{
  ...
  tick(){this.vy+=GRAV*.8;this.x+=this.vx;this.y+=this.vy;this.angle+=this.spin;this.vx*=.97;if(this.y>H+100)this.active=false;for(const p of map.plats){if(pInBox(this.x,this.y,p)){this.vy*=-.5;this.vx*=.5;this.spin*=.5;break;}}}
